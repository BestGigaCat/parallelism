...
public final class StudentAnalytics {
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
            final Student[] studentArray) {
        return Stream.of(studentArray).parallel().filter(a -> !a.checkIsCurrent())
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
            final Student[] studentArray) {
        List<Student> studentList = Arrays.asList(studentArray);
        return (int) studentList.stream().parallel()
                .filter(a -> (a.getAge() > 20 && a.getGrade() < 65 && !a.checkIsCurrent()))
                .count();
    }
}
